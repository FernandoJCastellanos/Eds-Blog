...
import { TotalBlogs } from "utils/getQuery";
import {
    Card,
    CardHeader,
    CardBody,
...
export const BlogPostTotal = () => {




    const { loading, error, data } = useQuery(TotalBlogs);
    if (loading) return <p>Loading...</p>;
    if (error) return <p>Error : {error.message}</p>;

    const totalBlogs = data.blogsposts.nodes
    ...
    return (
        <div className="flex flex-wrap justify-center gap-7 my-12">
            {totalBlogs.map((item) => (
